...
class landmarkPoseEstimation(Node):
    ...
    def __init__(self) -> None:
        ...
        self.raw_odometry_subs = self.create_subscription(Odometry, "odometry/raw", self.raw_odometry_subs, qos_profile)
        ...
        self.pose2D_pub = self.create_publisher(Float64MultiArray, 'landmark_updates', 10)
    ...
    def area3_reached_callback(self, msg:UInt8):
        if msg.data == 0xA5:
            ...
            self.pose2D[0]  = self.raw_odom_msg.pose.pose.position.x
            self.pose2D[1]  = self.raw_odom_msg.pose.pose.position.y
        ...
        if self.is_on_area3:
            if self.is_on_silo_region:     
                ...
                if ready_to_publish == True:
                    ...
                    print(f"raw_x: {self.raw_odom_msg.pose.pose.position.x}, raw_y: {self.raw_odom_msg.pose.pose.position.y}")
                    # print(f"isOnArea3: {self.is_on_area3}, isOnSiloRegion: {self.is_on_silo_region },isLandMarkDetected: {self.landMark_detected}")
                    print(f"pose_x: {self.pose2D[0]}, pose_y: {self.pose2D[1]}, {x_offset= }, {y_offset= }")
                    # breakpoint()
            ...
            if self.origin_reset_once == True:
                ...
                x_offset = self.pose2D[0]
                y_offset = self.pose2D[1]
                msg = Float64MultiArray()
                msg.data = [ float(CROSS_JUNCTION),
                            x_offset,
                            y_offset ]
                print("[Landmakr_pose_estimation_node]: origin reset once")
                # print("Update_odometry :: origin_reset_once")
                # print(f"isOnArea3: {self.is_on_area3}, isOnSiloRegion: {self.is_on_silo_region },isLandMarkDetected: {self.landMark_detected}")
                # print(f"pose2d_x: {self.pose2D[0]}, pose2d_y: {self.pose2D[1]}, {x_offset= }, {y_offset= }")
                # breakpoint()

                self.pose2D_pub.publish(msg)

        self.landMark_detected = False
        # print(f"isOnArea3: {self.is_on_area3}, isOnSiloRegion: {self.is_on_silo_region },isLandMarkDetected: {self.landMark_detected}, isOriginResetOnce: {self.origin_reset_once}, ballStoredNow: {self.ball_store_just_now}")
        return
